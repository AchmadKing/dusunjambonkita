/**
 * Dusun Jambon Official Portal - Main Script
 * Integrated Dynamic API Fetching & UI Interactive Logic
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------------------------
    // 4. Dynamic Data Fetching from PHP Backend
    // ----------------------------------------------------------------------

    // Helper to fetch with fallback endpoint
    ...
    async function loadBerandaData() {
        try {
            ...
            if (json && json.status === 'success' && json.data) {
                ...
                if (json.data.stats && json.data.stats.total_umkm !== undefined) {
                    const umkmCountEl = document.getElementById('berandaUmkmCountDisplay');
                    if (umkmCountEl) {
                        umkmCountEl.textContent = `${json.data.stats.total_umkm} Usaha`;
                    }
                }
            }
        } catch (err) {
            console.error('Error loading beranda:', err);
        }
    }
        ...
        try {
            ...
            if (json && json.status === 'success' && json.data) {
                ...
                if (Array.isArray(json.data.gallery)) {
                    renderGalleryAndSlideshow(json.data.gallery);
                }
            }
        } catch (err) {
        ...
        try {
            ...
            if (json && json.status === 'success' && json.data) {
                ...
                if (p) {
                    ...
                    if (petaTitle) petaTitle.textContent = p.map_title;

                    const petaDesc = document.getElementById('petaDescDisplay');
                    if (petaDesc) petaDesc.textContent = p.map_desc;
                ...
                if (Array.isArray(json.data.lokasi)) {
                    renderLocationGrid(json.data.lokasi);
                }
            }
        } catch (err) {
            console.error('Error loading administrasi:', err);
        }
    }
    ...
    // Initialize all Backend Data Loads
    loadBerandaData();
    loadProfilData();
    loadBeritaData();
